...
class payrollActions extends sfActions {
    ...
    public function executeStartProcess1(sfWebRequest $request) {
        ...
        $lastPay = $AdministrationService->getLastUnlockPayShedule();
...
//die($this->empName);
    ...
    public function executeUpdateEmployeePayrollInformation(sfWebRequest $request) {
        ...
        $Employee = $EmployeePayrollService->readEmployee($EmpID);
        ...
        if ($request->isMethod('post')) {
            ...
            try {
                ...
                $EmployeePayrollService->saveEmployee($Employee);
    ...
    public function executeConfiguration(sfWebRequest $request) {
        ...
        $this->DistrictList = $AdministrationService->getDistrict();
        $this->DivisionList = $AdministrationService->getDivisionByDistrict($Employee->hie_code_3);

        //if ($Employee->Users->def_level <= '2' || $Employee->Users->def_level == '4') {
        if ($Employee->Users->def_level == '2'){
            $MaxLockID = $AdministrationService->getMaxLockID($Employee->hie_code_3);
            $MaxUnlockID = $AdministrationService->getMaxUnlockID($Employee->hie_code_3);
...
//                    die(print_r($_POST));
                ...
                if ($chkApplytoAll == 1) {
                    ...
                    $AssignEmpService->saveElgToAll($transID, $transDetailID, $txtAppAllAmount, $txtFromDateApplyAll, $txtToDateApplyAll);
                } else {
                    $exploed = array();
                    $count_rows = array();
                    foreach ($_POST as $key => $value) {
...
//        die(print_r($BankDisketteDetail));
    ...
    public function executePayrollEmployeeList(sfWebRequest $request) {
        ...
        $result = $payProcessDao->PayrollEmployeeList($userCulture, $orderField, $orderBy, $reason, $type, $payroll, $payrolltype, $locationWise, $startDate, $endDate);
